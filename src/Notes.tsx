...
export function Notes({ state, dispatch }: NotesProps) {
    ...
    const note = state.notes?.find(x => x.date === state.editingDay);

    const text = note?.text ?? "";
    const pictures = note?.pictures ?? [];

    function setText(text: string) {
        dispatch({ type: "EDIT_NOTE", text });
    }
    ...
    async function addFiles(ev: React.ChangeEvent<HTMLInputElement>) {
        if (ev.target.files) {
            for (const file of Array.from(ev.target.files)) {
                const id = `pictures/${state.editingDay}/${
                    file.type
    ...
    function removePicture() {
        if (showingPicture) {
            dispatch({ type: "REMOVE_NOTE_PICTURE", id: showingPicture });
            setShowingPicture("");
        }
    }
